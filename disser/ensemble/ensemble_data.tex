...
Unfortunately, 119 time periods is too small a sample to meaningfully break into a training dataset and a verification dataset, especially when considering events that are expected to occur on fewer than 1\% of grid points.
For comparison, the NSSL-WRF training dataset had 4120 time periods and the verification dataset had 1425.
To compensate for this, a resampling approach was used.


From the list of 119 time periods, 59 of them were randomly chosen, without replacement, to be the training data and the remaining 60 were used as the verification data.
From the training data, $\sigma_x$, $\sigma_y$, $\theta$, $h$, and $k$ were calculated for each member of the SSEF.
These parameters were then used to produce calibrated forecasts for time periods in the verification dataset.
This process was repeated twenty times, giving a set of twenty ``simulations''.
